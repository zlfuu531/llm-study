...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .home-section h2 { font-size: clamp(22px, 2vw, 28px); }
    .home-section-note {
      max-width: 420px;
      margin: 0;
      color: var(--muted);
    ...
    .home-library-block summary::-webkit-details-marker { display: none; }
    .home-library-block summary::before {
      content: ">";
      display: inline-block;
      color: var(--quiet);
      margin-right: 7px;
    ...
    .home-library-block[open] summary::before { transform: rotate(90deg); }
    .home-library-count {
      color: var(--muted);
      font-size: 12px;
      font-weight: 650;
    ...
    @media (max-width: 760px) {
      ...
      .home-start-header p { text-align: left; }
      .home-section-header {
        display: block;
      }
      .home-section-note {
...
<body>
  ...
  <script>
    ...
    const defaultOpenGroups = new Set(['入口', '完整学习章']);
    ...
    const homeStartLinks = [
      { path: '00_两个月冲刺总路线.md', label: '先定路线', tag: '开始', note: '选岗位主线，先决定哪些少看。' },
      { path: 'modules/01_Transformer与Attention_完整学习章.md', label: '读 Transformer 完整章', tag: '核心', note: 'Attention、RoPE、KV Cache、复杂度先立住。' },
      { path: 'modules/02_训练微调对齐_完整学习章.md', label: '补训练与微调', tag: '核心', note: 'LoRA、LoRA+、DPO、ORPO、GRPO 放到一条线。' },
      { path: '03_高频题单100题.md', label: '刷高频题单', tag: '检测', note: '用题单找薄弱点，再跳答案版。' },
      { path: '06_公式与速查卡.md', label: '最后用速查卡收口', tag: '复盘', note: '面试前快速扫公式、指标和排查链路。' }
    ];
    const homeTracks = [
      {
        title: '不知道看什么：先走核心路线',
        label: '最推荐',
        note: '只看完整章和速查，不先钻扩展专题，适合打开页面不知道点哪的时候。',
        items: [
          ['00_两个月冲刺总路线.md', '路线'],
          ['modules/01_Transformer与Attention_完整学习章.md', 'Transformer'],
          ['modules/02_训练微调对齐_完整学习章.md', '训练对齐'],
          ['modules/03_RAG_Agent_MCP_完整学习章.md', 'RAG/Agent'],
          ['06_公式与速查卡.md', '速查']
        ]
      },
      {
        title: '大模型算法 / Post-training',
        label: '算法岗',
        note: '重点看结构公式、微调对齐、实验评测和训练系统，少被应用细枝末节拖走。',
        items: [
          ['modules/02_训练微调对齐_完整学习章.md', '训练对齐'],
          ['29_ReasoningPostTraining_RLVR_Verifier_TestTimeScaling面试.md', 'Reasoning'],
          ['28_ML_DL数学基础_损失函数优化器与指标面试.md', 'ML/DL'],
          ['17_大模型训练系统与分布式训练面试.md', '训练系统']
        ]
      },
      {
        title: 'AI 应用 / RAG Agent',
        label: '应用岗',
        note: '重点看 RAG、Agent、工具调用、评估、安全和项目表达。',
        items: [
          ['modules/03_RAG_Agent_MCP_完整学习章.md', 'RAG/Agent'],
          ['21_Agent工程化_ToolCalling与MCP面试.md', 'Agent 工程'],
      ...
      },
      {
        title: '推理部署 / AI Infra',
        label: 'Infra',
        note: '重点看推理引擎、GPU、量化、KV Cache、吞吐延迟和线上排查。',
        items: [
          ['modules/04_推理部署与性能优化_完整学习章.md', '推理'],
    ...
    function renderNav() {
      ...
      const shortcutHtml = q ? '' : '<div class="nav-start"><div class="nav-start-title">先看这些</div>' +
        navShortcuts.map(([path, label]) => {
          const doc = docByPath.get(path);
          if (!doc) return '';
          return '<button class="doc-link nav-shortcut' + (doc.path === currentPath ? ' active' : '') + '" type="button" data-path="' + escapeAttr(doc.path) + '">' + escapeHtml(label) + '</button>';
        }).join('') + '</div>';
      docNav.innerHTML = shortcutHtml + '<div class="nav-meta">' + filtered.length + ' / ' + docs.length + ' 个文档</div>' + Array.from(groups.entries()).map(([group, groupDocs]) => {
    ...
    function renderHomeStep(item) {
      const doc = docByPath.get(item.path);
      if (!doc) return '';
      return '<a class="home-step-link" href="#doc=' + encodeURIComponent(doc.path) + '" data-doc="' + escapeAttr(doc.path) + '">' +
        '<span class="home-step-main"><span class="home-step-title">' + escapeHtml(item.label) + '</span><span class="home-step-note">' + escapeHtml(item.note) + '</span></span>' +
        '<span class="home-step-tag">' + escapeHtml(item.tag) + '</span>' +
        '</a>';
    }

    function renderHomeTrack(track) {
      const links = track.items.map(([path, tag]) => renderHomeDocLink(path, tag)).filter(Boolean);
      if (!links.length) return '';
      return '<section class="home-track">' +
        '<div class="home-track-top"><h3>' + escapeHtml(track.title) + '</h3><span class="home-track-label">' + escapeHtml(track.label) + '</span></div>' +
        '<p>' + escapeHtml(track.note) + '</p>' +
        '<div class="home-links">' + links.join('') + '</div>' +
        '</section>';
    }

    function renderHomeLibraryBlock(section, index) {
      const links = section.items.map(([path, tag]) => renderHomeDocLink(path, tag)).filter(Boolean);
      if (!links.length) return '';
      const open = index === 0 ? ' open' : '';
      return '<details class="home-library-block"' + open + '>' +
        '<summary><span>' + escapeHtml(section.title) + '</span><span class="home-library-count">' + links.length + ' 个入口</span></summary>' +
        '<div class="home-library-body"><p class="home-library-note">' + escapeHtml(section.note) + '</p><div class="home-links">' + links.join('') + '</div></div>' +
        '</details>';
    }

    function renderHomePage() {
      const startLinks = homeStartLinks.map(renderHomeStep).filter(Boolean).join('');
      const trackCards = homeTracks.map(renderHomeTrack).filter(Boolean).join('');
      const library = homeSections.map(renderHomeLibraryBlock).filter(Boolean).join('');
      return '<div class="home-page">' +
        '<header class="home-hero">' +
          '<div>' +
            '<p class="home-kicker">少看一点，先看对的</p>' +
            '<h1 class="home-title">LLM 学习导航</h1>' +
            '<p class="home-summary">现在资料很多，所以主页只保留最重要的学习路径。先从推荐顺序或岗位主线进入；需要查专题时，再展开下面的专题库。</p>' +
          '</div>' +
          '<div class="home-stats" aria-label="资料概览">' +
            '<div class="home-stat"><span>文档</span><strong>' + docs.length + '</strong></div>' +
            '<div class="home-stat"><span>推荐起点</span><strong>' + homeStartLinks.length + '</strong></div>' +
            '<div class="home-stat"><span>岗位主线</span><strong>' + homeTracks.length + '</strong></div>' +
          '</div>' +
        '</header>' +
        '<section class="home-start" aria-label="推荐阅读顺序">' +
          '<div class="home-start-header"><h2>先看这 5 个</h2><p>如果你现在不知道点哪里，就按这个顺序走。不要一上来展开全部目录。</p></div>' +
          '<div class="home-start-list">' + startLinks + '</div>' +
        '</section>' +
        '<section class="home-section">' +
          '<div class="home-section-header">' +
            '<h2>按岗位选主线</h2>' +
            '<p class="home-section-note">每条主线只放最关键的入口。学完这些，再去专题库补细节。</p>' +
          '</div>' +
          '<div class="home-track-grid">' + trackCards + '</div>' +
        '</section>' +
        '<section class="home-section home-library">' +
          '<div class="home-section-header">' +
            '<h2>专题库</h2>' +
            '<p class="home-section-note">这里保留全量主题入口，默认折叠，查缺补漏时再展开。</p>' +
          '</div>' +
          '<div class="home-library-list">' + library + '</div>' +
        '</section>' +
