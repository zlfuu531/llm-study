...
这一章面向大模型算法、微调、Post-training、应用算法和训练工程岗位。它补的是“真实微调脚本怎么搭”的工程主线：Tokenizer 和 chat template 怎么处理，Datasets 怎么清洗和 map，Trainer/SFTTrainer/DPOTrainer/ORPOTrainer 什么时候用，PEFT/LoRA/QLoRA 怎么配，adapter 怎么保存和合并，以及线上评估和常见坑怎么排查。
...
- [modules/02_训练微调对齐_完整学习章.md](modules/02_训练微调对齐_完整学习章.md)：SFT、LoRA、QLoRA、DPO、ORPO、GRPO 理论主线。
...
-> Trainer / SFTTrainer / DPOTrainer / ORPOTrainer
...
| ORPO/单阶段偏好优化 | prompt + chosen + rejected | TRL ORPOTrainer |
...
## 11. DPOTrainer / ORPOTrainer 和偏好数据
...
- 希望用同一份 chosen/rejected 数据做单阶段 SFT + 偏好优化。
- 不想额外加载 reference model，降低流程和显存复杂度。
- 希望把 instruction tuning 和 preference tuning 更紧地合在一起。
...
- prompt 模板不一致。
- beta 太大或太小导致过强/过弱约束。
- ORPO 的 `lambda` 太大可能压偏好过猛，太小则接近普通 SFT。
...
Hugging Face 微调工程要按“数据 -> 模板 -> token -> loss -> 训练 -> 保存 -> 评估”讲。Tokenizer 和 chat template 是模型接口的一部分，SFT 数据要渲染成目标模型格式，并用 labels mask 只训练 assistant token。Datasets 负责 map/filter/shuffle/streaming，DataCollator 负责 padding 和 batch。Trainer 适合标准训练，SFTTrainer 更贴近 LLM 指令微调，DPOTrainer 处理 chosen/rejected 偏好数据并通常需要 reference model，ORPOTrainer 用 chosen/rejected 做 SFT + odds ratio 且不需要 reference model。PEFT/LoRA 通过 `LoraConfig` 选择 rank、alpha、target_modules 和 dropout，QLoRA 用 4-bit base model 加可训练 LoRA adapter 省显存。训练时常配 gradient checkpointing、`use_cache=False`、bf16、gradient accumulation。保存时要区分 adapter、merged model、tokenizer 和 model card。常见坑是 chat template 错、loss mask 错、target_modules 没匹配、packing/EOS 错、resume 不完整、DPO/ORPO 数据噪声和推理脚本与训练不一致。
